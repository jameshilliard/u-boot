...
static void mii_ephy_enable(struct phy_device *phydev)
{
...
static void ephy_config_default(struct phy_device *phydev)
{
	...
	phy_write(phydev, MDIO_DEVAD_NONE, 0x14, 0x708f);	/* PHYAFE TX optimization */
...
int genphy_config(struct phy_device *phydev)
{
	...
	if(val&sunxi_ac300_key)
	{
		...
		if(sid_value&0x200) { // if (ephy_cali & BIT(9))
