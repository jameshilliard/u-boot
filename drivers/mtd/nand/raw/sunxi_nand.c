...
static void sunxi_nfc_read_buf(struct mtd_info *mtd, uint8_t *buf, int len)
{
	...
	struct sunxi_nand_chip *sunxi_nand = to_sunxi_nand(nand);
	struct sunxi_nfc *nfc = to_sunxi_nfc(sunxi_nand->nand.controller);
	int sram_size = nfc->caps->sram_size ? nfc->caps->sram_size : NFC_SRAM_SIZE;
	int ret;
	int cnt;
	int offs = 0;
	u32 tmp;
...
static void sunxi_nfc_hw_ecc_enable(struct mtd_info *mtd)
{
	...
	writel(data->ecc_ctl, nfc->regs + NFC_REG_ECC_CTL);
}
...
static inline unsigned int sunxi_user_data_sz(struct sunxi_nfc *nfc, int step)
{
	if (nfc->caps->user_data_len)
		return nfc->caps->user_data_len(step);
				...
				struct nand_ecc_ctrl *ecc, int step)
{
	...
	for (i = 0; i < step; i++)
		ecc_off += sunxi_user_data_sz(nfc, i);
				       ...
				       bool bbm, int page)
{
	...
	int udata_len = sunxi_user_data_sz(nfc, step);
	u32 status;
	u32 pattern_found;
	int ret;
	...
	sunxi_nfc_set_user_data_len(nfc, udata_len, step);
	...
	*cur_off = oob_off + ecc->bytes + udata_len;
	...
	if (pattern_found & NFC_ECC_PAT_FOUND(step)) {
		u8 pattern = 0xff;
		...
		memset(oob, pattern, ecc->bytes + udata_len);

		return 1;
	}

	ret = NFC_ECC_ERR_CNT(step,
			      readl(nfc->regs + NFC_REG_ECC_ERR_CNT(nfc, step)));

	memcpy_fromio(data, nfc->regs + NFC_RAM0_BASE, ecc->size);

	nand->cmdfunc(mtd, NAND_CMD_RNDOUT, oob_off, -1);
	sunxi_nfc_randomizer_read_buf(mtd, oob, ecc->bytes + udata_len, true, page);
	...
	if (status & NFC_ECC_ERR(step)) {
		...
		if (nand->options & NAND_NEED_SCRAMBLING) {
			...
			nand->cmdfunc(mtd, NAND_CMD_RNDOUT, oob_off, -1);
			nand->read_buf(mtd, oob, ecc->bytes + udata_len);
		}

		ret = nand_check_erased_ecc_chunk(data,	ecc->size,
						  oob, ecc->bytes + udata_len,
						  NULL, 0, ecc->strength);
		if (ret >= 0)
			raw_mode = 1;
	} else {
		/*
		 * The engine protects user data bytes per chunk.
					    ...
					    bool randomize, int page)
{
	...
	struct sunxi_nfc *nfc = to_sunxi_nfc(nand->controller);
	struct nand_ecc_ctrl *ecc = &nand->ecc;
	int offset = sunxi_get_ecc_offset(nfc, ecc, ecc->steps);
					...
					int page)
{
	...
	/* Fill OOB data in */
	if (udata_len) {
		const u8 *user_data = oob;
		u8 user_data_buf[32];

		if (bbm && (nand->options & NAND_NEED_SCRAMBLING) &&
		    !nfc->caps->no_scramble_bbm) {
			memcpy(user_data_buf, oob, udata_len);
			sunxi_nfc_randomize_bbm(mtd, page, user_data_buf);
		...
		if (udata_len > 4)
			writel(sunxi_nfc_buf_to_user_data(user_data + 4),
			       nfc->regs + NFC_REG_USER_DATA(nfc, step + 1));
	}
				      ...
				      int oob_required, int page)
{
	...
	for (i = 0; i < ecc->steps; i++) {
		...
		int oob_off = sunxi_get_ecc_offset(nfc, ecc, i);
					       ...
					       int page)
{
	...
	for (i = 0; i < ecc->steps; i++) {
		unsigned int user_data_sz = sunxi_user_data_sz(nfc, i);
		int data_off = i * (ecc->size + ecc->bytes + user_data_sz);
		int oob_off = data_off + ecc->size;
		u8 *data = buf + (i * ecc->size);
		u8 *oob = chip->oob_poi + sunxi_get_ecc_offset(nfc, ecc, i);
						...
						int oob_required, int page)
{
	...
	for (i = 0; i < ecc->steps; i++) {
		...
		const u8 *oob = chip->oob_poi + sunxi_get_ecc_offset(nfc, ecc, i);
					      ...
					      struct nand_ecc_ctrl *ecc)
{
	...
	data->ecc_ctl = NFC_ECC_EN | NFC_ECC_MODE(nfc, data->mode) |
			NFC_ECC_EXCEPTION | NFC_ECC_PIPELINE;
	if (ecc->size == 512 && nfc->caps->has_ecc_block_512)
		data->ecc_ctl |= NFC_ECC_BLOCK_512;

	layout = &data->layout;
	nsectors = mtd->writesize / ecc->size;
	for (i = 0; i < nsectors; i++)
		total_user_data_sz += sunxi_user_data_sz(nfc, i);
	...
	for (i = 0; i < nsectors; i++) {
		...
		if (i) {
			layout->oobfree[i].offset =
				sunxi_get_ecc_offset(nfc, ecc, i);
			...
			layout->oobfree[i].length =
				(user_data_sz > 2) ? (user_data_sz - 2) : 0;
...
static unsigned int sunxi_user_data_len_vendor_h616(int step)
{
	/*
	 * In vendor image, the user data len is 8 byte on step 0 (that
	 * includes the BBM) and 0 bytes for the rest.
...
static unsigned int sunxi_user_data_len(int step)
{
	/*
	 * On A10/A23, this is the size of the NDFC User Data Register,
	 * containing the mandatory user data bytes following the ECC for each
	 * ECC step.
	 ...
	 *
	 * On H6/H616, this size became configurable, from 0 bytes to 32, via
	 * the USER_DATA_LEN registers.
	 * So the layout is:
...
static const struct sunxi_nfc_caps sunxi_nfc_a10_caps = {
	...
	.max_ecc_steps = 16,
	.sram_size = 1024,
	.user_data_len = &sunxi_user_data_len,
};
...
static const struct sunxi_nfc_caps sunxi_nfc_h616_caps = {
	...
	.user_data_len = &sunxi_user_data_len_vendor_h616,
};
